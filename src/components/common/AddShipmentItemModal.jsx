...
import { FiX, FiPlus, FiTrash2, FiAlertTriangle, FiUserPlus, FiActivity, FiSave, FiEdit3, FiSearch, FiCheck, FiChevronDown, FiFileText } from 'react-icons/fi';
import api, { shipmentService } from '../../services/api';
import PatientForm from '../forms/PatientForm'; 
import MedicationForm from '../forms/MedicationForm';
import { ClipLoader } from 'react-spinners'; 

export default function AddShipmentItemModal({ onClose, onSuccess, initialData, currentShipmentId }) {
  // --- ESTADOS DE DADOS ---
  ...
  // Items agora tem 'observation' e a lógica de busca individual
  const [items, setItems] = useState([{ name: '', quantity: 1, unit: 'CX', medicationId: null, observation: '' }]);
  
  // Controle de Sugestões (Autocomplete Manual)
  const [activeSearchIndex, setActiveSearchIndex] = useState(null); // Qual linha está sendo editada
  const [filteredSuggestions, setFilteredSuggestions] = useState([]); // Lista filtrada (max 20)
  ...
  const wrapperRef = useRef(null); // Para fechar a lista ao clicar fora
  ...
  useEffect(() => {
    if (initialData && patients.length > 0) {
        ...
        if (foundPatient) {
            ...
            setPatientSearchTerm(`${foundPatient.name} ${foundPatient.cpf ? `(${foundPatient.cpf})` : ''}`);
        }
  ...
  const loadInitialData = async () => {
      try {
        const [patRes, medRes] = await Promise.all([
           api.get('/patients'),
           api.get('/medications?limit=5000') // Traz tudo, mas só filtra na memória
        ]);
        ...
        else if (medData?.data && Array.isArray(medData.data)) setMedicationsList(medData.data);
  ...
  const handlePatientInput = (e) => {
      ...
      const match = patients.find(p => {
          const formatted = `${p.name} ${p.cpf ? `(${p.cpf})` : ''}`;
          return formatted === value || p.name === value;
      });
  ...
  // --- LÓGICA DE BUSCA INTELIGENTE (SEM TRAVAR) ---
  const handleNameChange = (index, value) => {
      const newItems = [...items];
      newItems[index].name = value;
      // Se alterou o nome manualmente, limpa o ID antigo para evitar inconsistência
      // Se o usuário digitar algo exato, o sistema vai aceitar como texto livre
      newItems[index].medicationId = null; 
      setItems(newItems);

      // Filtra apenas se tiver digitado algo
      if (value.trim().length > 0) {
          const lower = value.toLowerCase();
          // O segredo da performance: Slice(0, 30) pega só os 30 primeiros resultados
  ...
  const selectSuggestion = (index, med) => {
      ...
      setItems(newItems);
      setActiveSearchIndex(null); // Fecha a lista
  ...
  const checkDuplicity = async (medName, patientId) => {
      ...
      try {
          ...
          if (res.data.exists) {
              setDuplicityWarning(`⚠️ ATENÇÃO: "${medName}" já foi solicitado em ${new Date(res.data.date).toLocaleDateString()} (Remessa: ${res.data.code}).`);
          } else {
              setDuplicityWarning(null);
          }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedPatientId) return alert('Selecione um paciente válido da lista.');

    if (items.some(i => i.quantity <= 0)) return alert("Quantidade deve ser maior que zero.");
    ...
    try {
      ...
      if (!initialData) {
          ...
          setItems([{ name: '', quantity: 1, unit: 'CX', medicationId: null, observation: '' }]);
          setDuplicityWarning(null);
          ...
          <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4">
              <div className="bg-white rounded-xl w-full max-w-2xl shadow-2xl relative overflow-hidden animate-scale-in">
                   <div className="bg-gray-100 px-6 py-4 border-b flex justify-between items-center">
                       <h3 className="font-bold text-gray-700 flex items-center gap-2"><FiUserPlus/> Novo Paciente</h3>
                       <button onClick={() => setShowPatientForm(false)} className="hover:text-red-500 cursor-pointer"><FiX size={20}/></button>
                   </div>
                   <PatientForm 
      ...
      return (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4">
            <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl relative overflow-hidden animate-scale-in">
                <div className="bg-gray-100 px-6 py-4 border-b flex justify-between items-center">
                    <h3 className="font-bold text-gray-700 flex items-center gap-2"><FiActivity/> Nova Medicação</h3>
                    <button onClick={() => setShowMedForm(false)} className="hover:text-red-500 cursor-pointer"><FiX size={20}/></button>
                </div>
                 <MedicationForm 
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl p-0 relative animate-scale-in flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50 rounded-t-xl">
            <div>
                <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                    {initialData ? <><FiEdit3 className="text-blue-600"/> Editar Receita</> : <><FiPlus className="text-blue-600"/> Adicionar à Remessa</>}
                </h2>
                <p className="text-sm text-gray-500">
                    {initialData ? 'Altere os itens abaixo' : 'Busque o paciente e digite os itens'}
                </p>
            </div>
            <button onClick={onClose} className="p-2 hover:bg-gray-200 rounded-full transition cursor-pointer"><FiX size={24} /></button>
        </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
            {duplicityWarning && (
                <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 mb-6 flex items-start gap-3 animate-pulse">
                    ...
                    <div>
                        <p className="text-sm text-yellow-800 font-bold">Possível Duplicidade Detectada</p>
            ...
            {/* SELEÇÃO DE PACIENTE */}
            <div className="mb-6 bg-blue-50/50 p-4 rounded-lg border border-blue-100 relative">
                ...
                <div className="relative">
                    ...
                    <datalist id="patients-datalist">
                        {patients.map(p => (
                            <option key={p._id} value={`${p.name} ${p.cpf ? `(${p.cpf})` : ''}`} />
                        ))}
                    </datalist>
                </div>
                {selectedPatientId && (
            ...
            <div className="space-y-3" ref={wrapperRef}>
                <div className="flex justify-between items-center mb-1 px-1">
                    <label className="block text-sm font-bold text-gray-700">Medicamentos Prescritos</label>
                    <button 
                        type="button"
                ...
                <div key={index} className="grid grid-cols-[1.5fr_80px_100px_40px] gap-2 items-start bg-gray-50 p-2 rounded-lg border border-gray-200 transition-colors hover:border-blue-300 group">
                    
                    {/* COLUNA 1: NOME + OBSERVAÇÃO (Empilhados) */}
                    <div className="flex flex-col gap-1 relative">
                        ...
                        <input 
                            ...
                            onFocus={() => handleNameChange(index, item.name)} // Abre a lista ao focar
                            required
                            autoComplete="off"
                        />
                        
                        {/* LISTA DE SUGESTÕES (ABSOLUTA) */}
                        {activeSearchIndex === index && filteredSuggestions.length > 0 && (
                    ...
                    <div className="relative">
                         ...
                         />
                         <FiChevronDown className="absolute right-2 top-3 text-gray-400 pointer-events-none" size={12}/>
                    </div>
            ...
            <button
                type="button"
                onClick={() => setItems([...items, { name: '', quantity: 1, unit: 'CX', observation: '' }])}
        ...
        <div className="px-6 py-4 border-t bg-gray-50 rounded-b-xl flex justify-end gap-3">
            ...
            >
                {loading ? <ClipLoader size={20} color="#fff" /> : <><FiSave /> {initialData ? 'Atualizar' : 'Salvar'}</>}
            </button>
        </div>
